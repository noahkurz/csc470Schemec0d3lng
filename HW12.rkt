...
; (do-in-order c0d3*)
(define apply-parse-expression-to-list
  (lambda (list-of-c0d3)
    (if (null? list-of-c0d3)
        '()
        (cons (parse-expression(car list-of-c0d3))
              (apply-parse-expression-to-list (cdr list-of-c0d3))))))
...
(define do-in-order-helper
  (lambda(exp-to-eval env)
    (cond
      ((eq? exp-to-eval '())'())
      ((create-var-exp? (car exp-to-eval)) (do-in-order-helper (cdr exp-to-eval) (apply-expression (car exp-to-eval) env)))
      (else (cons(apply-expression (car exp-to-eval) env)(do-in-order-helper (cdr exp-to-eval) env)))
           )))
...
(define apply-expression
  (lambda (lcexp env)
    (cond
      ...
      ((do-in-order-exp? lcexp) (do-in-order-helper (do-in-order-exp->list-of-expressions lcexp) env))
      ;((do-in-order-exp? lcexp)(do-in-order-helper (do-in-order-exp->list-of-expressions lcexp) env))
...
(parse-expression '(do-in-order (literal 7)(remember t (literal 13)) (literal 8) (display (get-value t))))
