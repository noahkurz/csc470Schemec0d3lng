  ...
  (lambda (c0d3)
    (cond
      ((eq? (car c0d3) 'ask-question)
       (if-exp (bool-exp
                (test-exp->op (parse-expression (question-exp->test-exp c0d3))) (parse-expression(test-exp->left(question-exp->test-exp c0d3))) (parse-expression(test-exp->right(question-exp->test-exp c0d3))))
...
(define what-to-do
  (lambda(boolean-answer if-exp)
    (cond
      ((eq? boolean-answer #t)
       (if-exp->true-exp if-exp))
      ((eq? boolean-answer #f)
       (if-exp->false-exp if-exp)))))
...
(define c0d3-bool '(ask-question (test < (get-value c) (literal 7)) 'if-true-do-> (literal 1) 'if-false-do-> (literal 0)))
